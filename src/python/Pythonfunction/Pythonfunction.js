...
import fquestions from './fquestions.json'
...
import classNames from 'classnames/bind';
 ...
 class Pythonfunction extends Component {
     constructor(props){
         super(props);
         this.state = {
            fquestions,
     ...
     componentDidMount () {
         const {fquestions , currentQuestion , nextQuestion , previousQuestion } = this.state;
         this.displayQuestions(fquestions ,currentQuestion , nextQuestion , previousQuestion);
         this.startTimer();
     }
     ...
     displayQuestions =(
         fquestions = this.state.csquestions ,  
         currentQuestion, nextQuestion , previousQuestion
        ) => {
            let{ currentQuestionIndex} = this.state;
            if(!isEmpty(this.state.fquestions)){
                fquestions = this.state.fquestions;
                currentQuestion = fquestions[currentQuestionIndex];
                nextQuestion = fquestions[currentQuestionIndex + 1];
                previousQuestion = fquestions[ currentQuestionIndex - 1];
                const answer = currentQuestion.answer;
                this.setState({
                        currentQuestion , 
                        nextQuestion,
                        previousQuestion,
                        numberofQuestions:fquestions.length,
                        answer,
                        previousRandomNumbers: []
                } , () => {
                    this.showOptions();
        ...
        handleQuitButtonClick= () => {
                ...
                if(window.confirm('Are you sure you want to quit?')){
                    this.props.history.push('/pythoncourse');
        ...
        correctAnswers = () => {
            ...
            this.setState(prevState => ({
                ...
                if(this.state.nextQuestion === undefined){
                    ...
                    this.displayQuestions(this.state.fquestions , this.state.currentQuestion , this.state.nextQuestion , this.state.previousQuestion);
                }
        ...
        handleHints = () => {
           ...
           endGame = () => {
               ...
               setTimeout(() => {
                  this.props.history.push('/Pythonfsummary' , playStats) 
               }, 1000);
           }
