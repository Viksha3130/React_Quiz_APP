...
import cdquestions from './cdquestions.json'
...
import classNames from 'classnames/bind';
 ...
 class Cdatatype extends Component {
     constructor(props){
         super(props);
         this.state = {
            cdquestions,
     ...
     componentDidMount () {
         const {cdquestions , currentQuestion , nextQuestion , previousQuestion } = this.state;
         this.displayQuestions(cdquestions ,currentQuestion , nextQuestion , previousQuestion);
         this.startTimer();
     }
     ...
     displayQuestions =(
         cdquestions = this.state.cdquestions ,  
         currentQuestion, nextQuestion , previousQuestion
        ) => {
            let{ currentQuestionIndex} = this.state;
            if(!isEmpty(this.state.cdquestions)){
                cdquestions = this.state.cdquestions;
                currentQuestion = cdquestions[currentQuestionIndex];
                nextQuestion = cdquestions[currentQuestionIndex + 1];
                previousQuestion = cdquestions[ currentQuestionIndex - 1];
                const answer = currentQuestion.answer;
                this.setState({
                        currentQuestion , 
                        nextQuestion,
                        previousQuestion,
                        numberofQuestions:cdquestions.length,
                        answer,
                        previousRandomNumbers: []
                } , () => {
                    this.showOptions();
        ...
        correctAnswers = () => {
            ...
            this.setState(prevState => ({
                ...
                if(this.state.nextQuestion === undefined){
                    ...
                    this.displayQuestions(this.state.cdquestions , this.state.currentQuestion , this.state.nextQuestion , this.state.previousQuestion);
                }
        ...
        handleHints = () => {
           ...
           endGame = () => {
               ...
               setTimeout(() => {
                  this.props.history.push('/Cdatatypesummary' , playStats) 
               }, 1000);
           }
