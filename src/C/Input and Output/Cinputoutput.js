...
import Quizstyle from '../../components/Quizstyle.css'
import cioquestions from './cioquestions.json'
...
import classNames from 'classnames/bind';
 ...
 class Cinputoutput extends Component {
     constructor(props){
         super(props);
         this.state = {
            cioquestions,
     ...
     componentDidMount () {
         const {cioquestions , currentQuestion , nextQuestion , previousQuestion } = this.state;
         this.displayQuestions(cioquestions ,currentQuestion , nextQuestion , previousQuestion);
         this.startTimer();
     }
     ...
     displayQuestions =(
         cioquestions = this.state.cpsquestions ,  
         currentQuestion, nextQuestion , previousQuestion
        ) => {
            let{ currentQuestionIndex} = this.state;
            if(!isEmpty(this.state.cioquestions)){
                cioquestions = this.state.cioquestions;
                currentQuestion = cioquestions[currentQuestionIndex];
                nextQuestion = cioquestions[currentQuestionIndex + 1];
                previousQuestion = cioquestions[ currentQuestionIndex - 1];
                const answer = currentQuestion.answer;
                this.setState({
                        currentQuestion , 
                        nextQuestion,
                        previousQuestion,
                        numberofQuestions:cioquestions.length,
                        answer,
                        previousRandomNumbers: []
                } , () => {
                    this.showOptions();
        ...
        handleQuitButtonClick= () => {
                ...
                if(window.confirm('Are you sure you want to quit?')){
                    this.props.history.push('/Ccourse');
        ...
        correctAnswers = () => {
            ...
            this.setState(prevState => ({
                ...
                if(this.state.nextQuestion === undefined){
                    ...
                    this.displayQuestions(this.state.cioquestions , this.state.currentQuestion , this.state.nextQuestion , this.state.previousQuestion);
                }
        ...
        handleHints = () => {
           ...
           endGame = () => {
               ...
               setTimeout(() => {
                  this.props.history.push('/Ciosummary' , playStats) 
               }, 1000);
           }
