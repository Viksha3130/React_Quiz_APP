...
import cfquestions from './cfquestions.json'
...
import classNames from 'classnames/bind';
 ...
 class Ccontrolflow extends Component {
     constructor(props){
         super(props);
         this.state = {
            cfquestions,
     ...
     componentDidMount () {
         const {cfquestions , currentQuestion , nextQuestion , previousQuestion } = this.state;
         this.displayQuestions(cfquestions ,currentQuestion , nextQuestion , previousQuestion);
         this.startTimer();
     }
     ...
     displayQuestions =(
         cfquestions = this.state.cdquestions ,  
         currentQuestion, nextQuestion , previousQuestion
        ) => {
            let{ currentQuestionIndex} = this.state;
            if(!isEmpty(this.state.cfquestions)){
                cfquestions = this.state.cfquestions;
                currentQuestion = cfquestions[currentQuestionIndex];
                nextQuestion = cfquestions[currentQuestionIndex + 1];
                previousQuestion = cfquestions[ currentQuestionIndex - 1];
                const answer = currentQuestion.answer;
                this.setState({
                        currentQuestion , 
                        nextQuestion,
                        previousQuestion,
                        numberofQuestions:cfquestions.length,
                        answer,
                        previousRandomNumbers: []
                } , () => {
                    this.showOptions();
        ...
        correctAnswers = () => {
            ...
            this.setState(prevState => ({
                ...
                if(this.state.nextQuestion === undefined){
                    ...
                    this.displayQuestions(this.state.cfquestions , this.state.currentQuestion , this.state.nextQuestion , this.state.previousQuestion);
                }
        ...
        handleHints = () => {
           ...
           endGame = () => {
               ...
               setTimeout(() => {
                  this.props.history.push('/Ccfsummary' , playStats) 
               }, 1000);
           }
