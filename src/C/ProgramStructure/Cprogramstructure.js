...
import cpsquestions from './cpsquestions.json'
...
import classNames from 'classnames/bind';
 ...
 class Cprogramstructure extends Component {
     constructor(props){
         super(props);
         this.state = {
            cpsquestions,
     ...
     componentDidMount () {
         ...
         this.displayQuestions(cpsquestions ,currentQuestion , nextQuestion , previousQuestion);
         this.startTimer();
     }
     ...
     displayQuestions =(
         cpsquestions = this.state.cpsquestions ,  
         currentQuestion, nextQuestion , previousQuestion
        ) => {
            let{ currentQuestionIndex} = this.state;
            if(!isEmpty(this.state.cpsquestions)){
                cpsquestions = this.state.cpsquestions;
                currentQuestion = cpsquestions[currentQuestionIndex];
                nextQuestion = cpsquestions[currentQuestionIndex + 1];
                previousQuestion = cpsquestions[ currentQuestionIndex - 1];
                const answer = currentQuestion.answer;
                this.setState({
                        currentQuestion , 
                        nextQuestion,
                        previousQuestion,
                        numberofQuestions:cpsquestions.length,
                        answer,
                        previousRandomNumbers: []
                } , () => {
                    this.showOptions();
        ...
        correctAnswers = () => {
            ...
            this.setState(prevState => ({
                ...
                if(this.state.nextQuestion === undefined){
                    ...
                    this.displayQuestions(this.state.cpsquestions , this.state.currentQuestion , this.state.nextQuestion , this.state.previousQuestion);
                }
        ...
        handleHints = () => {
           ...
           endGame = () => {
               ...
               setTimeout(() => {
                  this.props.history.push('/Cpssummary' , playStats) 
               }, 1000);
           }
...
export default Cprogramstructure
